...
if __name__ == '__main__':
    ...
    hmm = HMM()
    stock_prices = get_stock_prices('VTL', '2017-09-01', f"{datetime.datetime.now():%Y-%m-%d}")
    ...
    print('\n')
    hmm = HMM()
    stock_prices = get_stock_prices('TTI', '2017-09-01', f"{datetime.datetime.now():%Y-%m-%d}")
    price_movement = get_price_movements(stock_prices)
    optimal_states = hmm.get_optimal_states(price_movement)
    prediction = hmm.predict(price_movement, optimal_states)
    if prediction==1:
    ...
    print('\n')
    hmm = HMM()
    stock_prices = get_stock_prices('RBBN', '2017-09-01', f"{datetime.datetime.now():%Y-%m-%d}")
    price_movement = get_price_movements(stock_prices)
    optimal_states = hmm.get_optimal_states(price_movement)
    prediction = hmm.predict(price_movement, optimal_states)
    if prediction==1:
